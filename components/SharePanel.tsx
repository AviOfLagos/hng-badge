...
interface Platform {
  id: string;
  name: string;
  hint: string;       // shown under the name at idle
  doneMsg: string;    // shown after successful share
  color: string;
  hoverColor: string;
  borderColor: string;
...
const PLATFORMS: Platform[] = [
  ...
  },
  {
    id: "whatsapp",
    name: "WhatsApp",
    hint: "Opens WhatsApp with text ready",
  ...
  },
  {
    id: "instagram",
    name: "Instagram",
    hint: "Caption copied — paste in app",
...
async function getBadgeFile(dataUrl: string): Promise<File> {
  const blob = await fetch(dataUrl).then((r) => r.blob());
  return new File([blob], "hng-badge.png", { type: "image/png" });
}
  ...
  const handleShare = useCallback(
    async (platform: Platform) => {
      ...
      try {
        ...
        // ── Mobile: use native Web Share sheet ──
        const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
        if (isMobile && "share" in navigator) {
          try {
            const file = await getBadgeFile(dataUrl);
        ...
        // ── Desktop: open platform URL synchronously FIRST (must be
        //    before any await to avoid popup blockers), then download ──
        if (targetUrl) {
    ...
    [role, badgeName, canvasRef]
  );

  // track which platform just fired so we can show its done message
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        {PLATFORMS.filter((p) => p.id === "x").map((platform) => {
            ...
            >
              ...
              <span
                className={`flex-shrink-0
                  ...
                  ${platform.id === "snapchat" ? "text-[#FFFC00]" : ""}
                `}
              >
                {platform.icon}
              </span>
              ...
              <span className="flex flex-col items-start min-w-0">
                <span className="text-white text-xs font-semibold leading-tight">
                  {status === "loading" ? "Preparing…" : platform.name}
                </span>
                {status === "idle" && (
                  <span className="text-gray-500 text-[10px] leading-tight mt-0.5 truncate w-full">
                    {platform.hint}
                  </span>
                )}
              </span>
      ...
      {/* Done message — shows what just happened */}
      <div className={`transition-all duration-300 overflow-hidden ${activeDoneMsg ? "max-h-10 opacity-100" : "max-h-0 opacity-0"}`}>
        <p className="text-[12px] text-green-400 font-medium py-1">
          {activeDoneMsg}
        </p>
      </div>

      {/* Static note for image attachment */}
      {!activeDoneMsg && (
        <p className="text-[11px] text-gray-600 leading-relaxed">
          Badge saves to your device on every tap — attach it when the app opens.
        </p>
      )}
    </div>
  );
